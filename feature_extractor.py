...
class Deep_fingerprinting(nn.Module):
    ...
    def __init__(self, feature_width=100, nb_classes=None):
        ...
        self._1Conv1D = nn.Conv1d(stride=conv_stride_size[1], kernel_size=kernel_size[1], in_channels=1, out_channels=filter_num[1], padding=kernel_size[1] // 2)
        self._2BatchNormalization = nn.BatchNorm1d(filter_num[1])
        self._3ELU = nn.ELU(alpha=1.0)
        self._4Conv1D = nn.Conv1d(in_channels=filter_num[1], out_channels=filter_num[1], kernel_size=kernel_size[1], stride=conv_stride_size[1], padding=kernel_size[1] // 2)
        ...
        self._9Conv1D = nn.Conv1d(in_channels=filter_num[1], out_channels=filter_num[2], kernel_size=kernel_size[2], stride=conv_stride_size[2], padding=kernel_size[2] // 2)
        self._10BatchNormalization = nn.BatchNorm1d(filter_num[2])
        self._11Relu = nn.ReLU()
        self._12Conv1D = nn.Conv1d(in_channels=filter_num[2], out_channels=filter_num[2], kernel_size=kernel_size[2], stride=conv_stride_size[2], padding=kernel_size[2] // 2)
        self._13BatchNormalization = nn.BatchNorm1d(filter_num[2])
        self._14Relu = nn.ReLU()
        self._15MaxPooling1D = nn.MaxPool1d(kernel_size=pool_size[2], stride=pool_stride_size[2], padding=pool_size[2] // 2)
        self._16Dropout = nn.Dropout(p=0.1)
        self._17Conv1D = nn.Conv1d(in_channels=filter_num[2], out_channels=filter_num[3], kernel_size=kernel_size[3], stride=conv_stride_size[3], padding=kernel_size[3] // 2)
        self._18BatchNormalization = nn.BatchNorm1d(filter_num[3])
        self._19Relu = nn.ReLU()
        self._20Conv1D = nn.Conv1d(in_channels=filter_num[3], out_channels=filter_num[3], kernel_size=kernel_size[3], stride=conv_stride_size[3], padding=kernel_size[3] // 2)
        self._21BatchNormalization = nn.BatchNorm1d(filter_num[3])
        self._22Relu = nn.ReLU()
        self._23MaxPooling1D = nn.MaxPool1d(kernel_size=pool_size[3], stride=pool_stride_size[3], padding=pool_size[3] // 2)
        self._24Dropout = nn.Dropout(p=0.1)
        self._25Conv1D = nn.Conv1d(in_channels=filter_num[3], out_channels=filter_num[4], kernel_size=kernel_size[4], stride=conv_stride_size[4], padding=kernel_size[4] // 4)
        self._26BatchNormalization = nn.BatchNorm1d(filter_num[4])
        self._27Relu = nn.ReLU()
        self._28Conv1D = nn.Conv1d(in_channels=filter_num[4], out_channels=filter_num[4], kernel_size=kernel_size[4], stride=conv_stride_size[4], padding=kernel_size[4] // 2)
        self._29BatchNormalization = nn.BatchNorm1d(filter_num[4])
        self._30Relu = nn.ReLU()
        self._31MaxPooling1D = nn.MaxPool1d(kernel_size=pool_size[4], stride=pool_stride_size[4], padding=pool_size[4] // 2)
