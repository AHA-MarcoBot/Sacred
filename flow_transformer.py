...
class FlowInterAttention(nn.Module):
    """流间 Multi-Head Attention：流与流之间互相观察"""
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: (batch, max_flows, d_model) 每个流的特征向量
            mask: (batch, max_flows) bool mask，True表示有效流
        Returns:
            output: (batch, max_flows, d_model) 流间交互后的特征
        """
        B, F, D = x.shape
        ...
        # QKV projection
        qkv = self.qkv(x).reshape(B, F, 3, self.num_heads, self.head_dim)
        qkv = qkv.permute(2, 0, 3, 1, 4)  # (3, B, heads, F, head_dim)
        q, k, v = qkv[0], qkv[1], qkv[2]

        # Attention scores
        attn_scores = (q @ k.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (B, heads, F, F)

        # Apply mask if provided (mask out invalid flows)
        if mask is not None:
            # mask: (B, F) -> (B, 1, F, F) for broadcasting
            # 无效流不能作为key/value被关注，也不能作为query关注其他流
            # 创建 (B, 1, F, F) 的mask，其中 mask[i, j] = True 表示可以关注
            mask_expanded = mask.unsqueeze(1).unsqueeze(-1)  # (B, 1, F, 1)
            mask_2d = mask_expanded.float() * mask_expanded.transpose(-2, -1).float()  # (B, 1, F, F)

            # 创建对角线mask：确保有效流至少能关注自己
            diag_eye = torch.eye(F, device=mask.device, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # (1, 1, F, F)
            diag_mask = diag_eye * mask_expanded.float()  # (B, 1, F, F)，只有有效流且是自己时才为1

            # 合并mask：允许关注的位置 = (query有效 AND key有效) OR (query有效 AND 是自己)
            mask_2d = torch.clamp(mask_2d + diag_mask, 0, 1)  # (B, 1, F, F)

            # 将无效位置的注意力分数设为-inf
            attn_scores = attn_scores.masked_fill(mask_2d == 0, float('-inf'))

            # 确保有效流至少能关注自己：向量化修复对角线位置
            # 提取对角线值
            diag_values = torch.diagonal(attn_scores, dim1=-2, dim2=-1)  # (B, heads, F)
            # 创建mask：有效流且对角线是-inf的位置
            mask_flat = mask.float().unsqueeze(1)  # (B, 1, F)
            is_inf_diag = torch.isinf(diag_values) & (diag_values < 0)  # (B, heads, F)
            should_fix = is_inf_diag & (mask_flat > 0)  # (B, heads, F)

            # 修复对角线：将-inf改为0
            diag_values = torch.where(should_fix, torch.zeros_like(diag_values), diag_values)

            # 将修复后的对角线值写回attn_scores（向量化）
            # 使用 diagonal 的视图直接修改
            diag_view = torch.diagonal(attn_scores, dim1=-2, dim2=-1)  # 获取对角线视图
            diag_view.copy_(diag_values)  # 直接复制，避免循环

        # Softmax
        # 在softmax之前，对于无效流的query，将所有key设为-inf，但将对角线设为0
        # 这样可以防止softmax产生NaN（因为至少有一个位置不是-inf）
        if mask is not None:
            # 向量化处理：找到哪些query是无效的
            invalid_queries = ~mask  # (B, F)，True表示无效流

            # 将这些无效query对应的所有key设为-inf（向量化）
            # attn_scores shape: (B, heads, F, F)
            # 需要将 invalid_queries[b, f] == True 的所有 attn_scores[b, :, f, :] 设为-inf
            invalid_expanded = invalid_queries.unsqueeze(1).unsqueeze(-1)  # (B, 1, F, 1)
            attn_scores = attn_scores.masked_fill(invalid_expanded, float('-inf'))

            # 然后将无效query的对角线设为0，确保softmax有输出（向量化）
            # 创建对角线索引mask：对于无效流，将对角线位置设为0
            # invalid_queries: (B, F) -> 扩展到 (B, heads, F, F)
            invalid_expanded_full = invalid_queries.unsqueeze(1).unsqueeze(-1)  # (B, 1, F, 1)
            diag_eye = torch.eye(F, device=mask.device, dtype=torch.bool).unsqueeze(0).unsqueeze(0)  # (1, 1, F, F)
            # 只有无效流且是对角线位置时才为True
            invalid_diag_mask = invalid_expanded_full & diag_eye  # (B, 1, F, F)
            attn_scores = attn_scores.masked_fill(invalid_diag_mask, 0.0)

        attn_weights = nnF.softmax(attn_scores, dim=-1)

        # 如果mask存在，确保无效位置的权重为0（防止NaN，并确保只关注有效流）
        if mask is not None:
            mask_expanded = mask.unsqueeze(1).unsqueeze(-1)  # (B, 1, F, 1)
            mask_2d = mask_expanded.float() * mask_expanded.transpose(-2, -1).float()  # (B, 1, F, F)
            diag_eye = torch.eye(F, device=mask.device, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
            diag_mask = diag_eye * mask_expanded.float()
            mask_2d = torch.clamp(mask_2d + diag_mask, 0, 1)
            attn_weights = attn_weights * mask_2d

            # 确保无效流的权重为0（防止NaN传播）
            mask_flat = mask.float().unsqueeze(1).unsqueeze(-1)  # (B, 1, F, 1)
            attn_weights = attn_weights * mask_flat  # 无效流的query权重为0
            attn_weights = attn_weights * mask_flat.transpose(-2, -1)  # 无效流的key权重为0

        attn_weights = self.dropout(attn_weights)

        # Apply attention to values
        out = attn_weights @ v  # (B, heads, F, head_dim)
        out = out.transpose(1, 2).reshape(B, F, D)  # (B, F, D)
...
class FlowSequenceTransformer(nn.Module):
    ...
    def __init__(
        ...
        device='cuda',
        mtu=1500.0,
        use_positional_encoding=True,      # 新增：位置编码消融标志
        use_window_attention=True,          # 新增：窗口注意力消融标志
        use_flow_inter_attention=True,       # 新增：流间注意力消融标志
    ):
        super().__init__()
        ...
        self.use_flow_inter_attention = use_flow_inter_attention
        ...
        self.ffn = FeedForward(d_model, d_model * 4, dropout=dropout)  # 时序FFN

        # ========== 流间注意力层 ==========
        if use_flow_inter_attention:
            self.flow_inter_attn = FlowInterAttention(
                d_model=d_model,
                num_heads=8,
                dropout=dropout
            )
            self.flow_inter_norm = nn.LayerNorm(d_model)
            self.flow_inter_ffn = FeedForward(d_model, d_model * 4, dropout=dropout)  # 流间FFN（独立）
        else:
            self.flow_inter_attn = None
            self.flow_inter_norm = None
            self.flow_inter_ffn = None
    ...
    def forward(self, flow_sequence, num_flows):
        ...
        # ========== Window Attention（时序处理）==========
        if self.use_window_attention and self.window_attn is not None:
        x = x + self.window_attn(self.norm1(x))
        else:
            # 消融版本：跳过窗口注意力，只应用归一化
            x = self.norm1(x)

        x = x + self.ffn(self.norm2(x))  # 使用时序FFN
        ...
        # ========== 流间 Multi-Head Attention（新增）==========
        # 此时 x 形状为 (B, max_flows, 64)
        # flow_mask 形状为 (B, max_flows)，True表示有效流
        if self.use_flow_inter_attention and self.flow_inter_attn is not None:
            x = self.flow_inter_attn(x, mask=flow_mask)  # 流与流之间互相观察
            x = x + self.flow_inter_ffn(self.flow_inter_norm(x))  # 使用流间FFN（独立）
        else:
            # 消融版本：跳过流间注意力
            pass
...
if __name__ == "__main__":
    print("=" * 80)
    print("Testing FlowSequenceTransformer with Flow Inter-Attention")
    print("=" * 80)
