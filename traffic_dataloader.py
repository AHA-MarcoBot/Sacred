...
def load_spilt_dataset(data_path: str, split_ratio: float=0.2, random_seed: int=42) -> Tuple[Dict, Dict]:
    ...
    train_dataset = {'packet_length': packet_length[train_indices], 'arrive_time_delta': arrive_time_delta[train_indices], 'start_timestamp': start_timestamp[train_indices], 'end_timestamp': end_timestamp[train_indices], 'labels': labels[train_indices]}
    test_dataset = {'packet_length': packet_length[test_indices], 'arrive_time_delta': arrive_time_delta[test_indices], 'start_timestamp': start_timestamp[test_indices], 'end_timestamp': end_timestamp[test_indices], 'labels': labels[test_indices]}
    print(f'数据集划分完成:')
...
def get_dataset_statistics(dataset: Dict) -> Union[str, Dict]:
    ...
    stats_dict = {'num_samples': num_samples, 'num_classes': num_classes, 'class_distribution': {int(label): int(count) for label, count in zip(unique_labels, label_counts)}, 'packet_length_stats': {'mean': float(np.mean(packet_length_stats)), 'std': float(np.std(packet_length_stats)), 'min': int(np.min(packet_length_stats)), 'max': int(np.max(packet_length_stats)), 'median': float(np.median(packet_length_stats)), 'total_packets': int(total_packets)}, 'time_delta_stats': {'mean': float(np.mean(time_delta_stats[time_delta_stats > 0])) if np.any(time_delta_stats > 0) else 0.0, 'std': float(np.std(time_delta_stats[time_delta_stats > 0])) if np.any(time_delta_stats > 0) else 0.0, 'min': float(np.min(time_delta_stats[time_delta_stats > 0])) if np.any(time_delta_stats > 0) else 0.0, 'max': float(np.max(time_delta_stats)), 'median': float(np.median(time_delta_stats[time_delta_stats > 0])) if np.any(time_delta_stats > 0) else 0.0, 'total_time': float(np.sum(time_delta_stats))}}
    if start_timestamp is not None and end_timestamp is not None:
        duration = end_timestamp - start_timestamp
        stats_dict['timestamp_stats'] = {'start_min': float(np.min(start_timestamp)), 'start_max': float(np.max(start_timestamp)), 'end_min': float(np.min(end_timestamp)), 'end_max': float(np.max(end_timestamp)), 'duration_mean': float(np.mean(duration)), 'duration_std': float(np.std(duration)), 'duration_min': float(np.min(duration)), 'duration_max': float(np.max(duration))}
    print('\n' + '=' * 60)
    print('数据集统计信息')
    print('=' * 60)
